...
public class HumanParser implements JsonParser<Human> {
    @Override
    public Human getObjFromJsonObj(String str) {
        ...
        List<Pet> listOfPet = new PetParser().getArrayFromJsonArray(humanMap.get("listOfPet"));
    ...
    public List<Human> getArrayFromJsonArray(String jsonArr) {
        ...
        for (int i = 0; i < humanMapArray.size(); i++) {
            ...
            if (humanMapArray.get(i).get("listOfPet") != null) {
                List<Pet> listOfPet = new PetParser().getArrayFromJsonArray(humanMapArray.get(i).get("listOfPet"));
                ryanGosling.setListOfPet(listOfPet);
            }
